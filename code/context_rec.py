...
def main(seed, llm_model="microsoft/phi-3-mini-4k-instruct"):
    ...
    # Only embedding files, never *_text.pkl
    profile_files = sorted(
        p for p in data_dir.glob(f"bert_long_term_user_profiles_{cfg.model_name_short}_N*_T*.pkl")
    ...
    if not profile_files:
        ...
        if old_file.exists():
            ...
            raise FileNotFoundError(
                f"No embedding profile files found for model {llm_model} in {data_dir}\n"
                f"Run: python preprocessing/generate_profiles.py --llm_model {llm_model}"
            )
    else:
        # Prefer the most recently modified embedding file
        cfg.USER_LONG_TERM_PATH = str(max(profile_files, key=lambda p: p.stat().st_mtime))
