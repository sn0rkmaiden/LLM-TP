"""
Description:
    In this experiment, we are using the short-/long-term user profiles that have been created based on
    centric approach over the BERT features of items that have been interacted by the user.
    Other part of the architecture including Attention Network and MLP is the same as proposed method.
    In fact, in this experiment we don't leverage LLM to create user profiles.
"""
...
DATASET = 'games'
SCRIPT_NAME = "temp_fusion.py"
METHOD_NAME = "Temp-Fusion"
...
class RecSysDataset(Dataset):
    ...
    def __init__(
        ...
        Args:
            ...
            user_short_term_dict (dict): user_id -> short-term 384-dim embedding.
            user_long_term_dict (dict): user_id -> long-term 384-dim embedding.
    ...
    def __getitem__(self, idx):
        """
        Return:
            user_short_term_emb: float tensor [384]
            user_long_term_emb:  float tensor [384]
        ...
        data = []
        data.append((
            np.array(user_short_term_emb, dtype=np.float32),
            np.array(user_long_term_emb, dtype=np.float32),
...
def collate_fn(batch):
    ...
    Each element in batch is itself a list of (u_s, u_l, item, label) of length (num_neg_samples+1).
...
class AttentionLayer(nn.Module):
    """
    A simple attention layer to combine short-term and long-term user representations.
    We learn an attention score that weights each representation.

    Alternatively, you could implement multi-headed attention or more advanced versions.
    Here, we do a basic feed-forward network to produce attention weights.
    """
    ...
    def forward(self, short_term_emb, long_term_emb):
        """
        short_term_emb: [batch_size, embed_dim]
        long_term_emb:  [batch_size, embed_dim]

        Returns:
            user_rep: Weighted combination of short_term_emb and long_term_emb
                      shape: [batch_size, embed_dim]
        """
        # Stack embeddings for attention: shape => [batch_size, 2, embed_dim]
        combined = torch.stack([short_term_emb, long_term_emb], dim=1)  # [B, 2, E]

        # Flatten last dimension for input to fc => we do it over each embedding
        # We'll produce attention weights for each of the two embeddings.
        B, N, E = combined.size()  # N=2
        # Reshape to [B*N, E]
        combined_reshape = combined.view(B * N, E)  # [B*2, E]

        # Compute attention logits => [B*2, 1]
        attn_logits = self.fc(combined_reshape)

        # Reshape back => [B, 2]
        attn_logits = attn_logits.view(B, N)

        # Softmax over the 2 embeddings
        attn_weights = self.softmax(attn_logits)  # [B, 2]

        # Weighted sum
        # Expand attn_weights to broadcast => [B, 2, 1]
        attn_weights = attn_weights.unsqueeze(-1)  # [B, 2, 1]

        # Weighted combination => [B, E]
...
class RecSysModel(nn.Module):
    ...
    Recommendation model that:
        1) Takes user_short_term_emb, user_long_term_emb, and item_emb
        2) Uses an attention layer to combine user_short_term_emb and user_long_term_emb
        3) Computes the matching score with item_emb
        4) Outputs a probability for classification
    """
    ...
    def __init__(self, embed_dim=384, hidden_dim=128, dropout=0.2):
        ...
        # Final feed-forward to produce logit
    ...
    def forward(self, user_s_emb, user_l_emb, item_emb):
        # 1) Use attention to get user representation
        user_rep = self.attention(user_s_emb, user_l_emb)  # [B, E]

        # 2) Concatenate with item_emb
        x = torch.cat([user_rep, item_emb], dim=1)  # [B, 2*E]

        # 3) Pass through final fc
...
def evaluate_ranking(
    ...
    for user_id, test_items in user_to_test_items.items():
        ...
        user_s_np = np.array(user_s_dict[user_id], dtype=np.float32)
        user_l_np = np.array(user_l_dict[user_id], dtype=np.float32)
        ...
        # Compute user representation (via attention)
        with torch.no_grad():
        ...
        user_rep_expanded = user_rep.repeat(item_embs_torch.size(0), 1)  # [num_items, 384]
...
def main(seed):
    ...
    seed_label = "none" if seed is None else str(seed)
    checkpoint_path = checkpoint_dir / f"temp_fusion_{METHOD_NAME.lower()}_{DATASET}_seed{seed_label}.pt"
